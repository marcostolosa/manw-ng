...
class ContinuousURLValidator:
    ...
    def generate_possible_urls(self, function_name: str, header: str) -> List[str]:
        ...
        if any(prefix in func_lower for prefix in ['zw', 'rtl', 'io', 'mm', 'ke']):
            base_patterns.append(f"https://learn.microsoft.com/en-us/windows-hardware/drivers/ddi/{header_lower}/nf-{header_lower}-{func_lower}")
        
        return base_patterns
    
    def discover_working_url(self, function_name: str, predicted_header: str) -> Optional[str]:
                ...
                if working_url:
                    ...
                    if '/api/' in working_url:
                        ...
                        if len(parts) >= 1:
                            ...
                            if func_name.lower() not in self.current_mappings or self.current_mappings[func_name.lower()] != actual_header:
    ...
    def update_mappings(self, new_mappings: Dict[str, str]) -> int:
        ...
        for func, header in new_mappings.items():
            if func not in self.current_mappings or self.current_mappings[func] != header:
    ...
    def generate_report(self) -> str:
        ...
        success_rate = (self.validated_count / total_tested) * 100 if total_tested > 0 else 0
