...
Examples:
  ...
  manw-ng.py --output markdown RegOpenKeyEx   # Markdown output
    ...
    try:
        ...
        if args.output == "rich":
            try:
                formatter = RichFormatter(language=args.language, show_remarks=args.obs, show_parameter_tables=args.tabs)
            ...
            print(formatter.format_output(function_info, show_remarks=args.obs, show_parameter_tables=args.tabs))
        elif args.output == "markdown":
            formatter = MarkdownFormatter()
            print(
                formatter.format_output(
                    function_info, language=args.language, show_remarks=args.obs, show_parameter_tables=args.tabs
                )
            )
...
def handle_exec_fast():
    ...
    abbrevs = {'k': 'kernel32.dll', 'u': 'user32.dll', 'n': 'ntdll.dll', 'a32': 'advapi32.dll'}
    resolved_dll = abbrevs.get(dll_name.lower(), dll_name if '.' in dll_name else f"{dll_name}.dll")
    ...
    try:
        ...
        for arg in args:
            ...
            try:
                val = int(arg, 0)
                parsed_args.append(ct.c_uint32(val) if val <= 0xFFFFFFFF else ct.c_uint64(val))
            except ValueError:
