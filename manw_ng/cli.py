...
# -*- coding: utf-8 -*-
"""
MANW-NG CLI: Win32 API Documentation Scraper + WinAPI Execution Engine
...
def handle_exec_fast():
    """Ultra-fast WinAPI execution: manw-ng.py exec dll:function [args...] [options]"""
    if len(sys.argv) < 3:
        print(
            "Usage: manw-ng.py exec dll:function [args...] "
            "[--ret void|u32|i32|u64|i64|bool|ptr] [--wide] [--show-error]"
        )
        sys.exit(1)

    dll_func = sys.argv[2]
    raw_tokens = sys.argv[3:]
    ...
    while i < len(raw_tokens):
        ...
        if token == "--ret":
            i += 1
            if i >= len(raw_tokens):
                print("Error: --ret requires a value")
                sys.exit(1)
            ret_type = raw_tokens[i]
        elif token == "--wide":
            wide = True
        elif token == "--show-error":
            show_error = True
        else:
            call_args.append(token)
        i += 1

    from manw_ng.execution import ExecutionEngine, ExecutionError
    ...
    if result.buffer_dump:
        print()
        print(result.buffer_dump)
    if show_error:
        print(f"GetLastError: {result.last_error_code} ({result.last_error_message})")
    print(f"Executed in {result.elapsed_seconds:.3f}s")
