...
class WinAPIExecutor:
    ...
    def load_module(self, module_name: str) -> wt.HMODULE:
        ...
        try:
            ...
            if not module_handle:
                ...
                raise OSError(
                    f"Failed to load {resolved_name}: {error_msg} (Error: {error_code})"
                )
    ...
    def resolve_function(
        self, module_name: str, function_name: str, force_wide: bool = False
    ) -> Tuple[wt.HMODULE, Any, str]:
        """
        ...
        for candidate in candidates:
            try:
                addr = self.kernel32.GetProcAddress(
                    module_handle, candidate.encode("ascii")
                )
                if addr:
                    function_addr = addr
    ...
    def _build_function_prototype(
        self, function_addr: Any, signature: FunctionSignature
    ) -> Any:
        ...
        try:
            with self._execution_context():
                ...
                signature = FunctionSignature(
                    function_name, dll_spec, args, return_type
                )
