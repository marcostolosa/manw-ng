...
class Win32APIScraper:
    ...
    def scrape_function(self, function_name: str) -> Dict:
        ...
        """
        if not self.quiet:
            self.console.print(f"[cyan]Buscando documentação para: {function_name}[/cyan]")

        # Try direct URL first (fastest)
        direct_url = self._try_direct_url(function_name)
        if direct_url:
            if not self.quiet:
                self.console.print(f"[green]Documentação encontrada:[/green] {self._format_url_display(direct_url)}")
        ...
        if not self.quiet and search_results:
            with Status("", console=self.console) as status:
                for i, url in enumerate(search_results, 1):
                    try:
                        status.update(f"[bold blue]\\[{i}/{len(search_results)}] Testando:[/bold blue] {self._format_url_display(url)}")
                        result = self._parse_function_page(url, status)
                        
                        status.stop()
                        self.console.print(f"[green]Documentação encontrada:[/green] {self._format_url_display(url)}")
                        return result
